...
Empirical studies in accounting and finance assume that the market not only incorporates all available information into price, but also forms precise and efficient estimates of individual valuation parameters such as the persistence of earnings. However, prevailing models of investor learning do not provide a mechanism by which investors distinguish between multiple parameters when observing aggregate price movements. This paper extends the model in citet:grossmanImpossibilityInformationallyEfficient1980 to accommodate multiple parameters and shows that this creates an identification problem: a single price cannot uniquely determine $n$ individual parameters when $n > 1$. This constraint binds even under Bayesian optimality: a fully rational investor's posterior variance for each component remains $\sigma_0^2(n-1)/n$, approaching the prior variance as $n$ grows. Formal results show that non-identification forces investors to apply average persistence to the aggregate, generating forecast errors that increase in both the number of components and the dispersion of their persistence rates. Each supplementary signal --- a disclosure, an analyst report --- that reveals a component value reduces these errors, with the largest reductions coming from components whose persistence is most distant from the average of the remaining unknowns. An agent-based simulation confirms these results under bounded rationality. Aggregate market efficiency therefore does not imply efficient pricing of the individual components that make up that aggregate.
...
* Introduction
  ...
  The title alludes to a reversal: in existing models, ``noise traders'' are irrational agents whose presence prevents price from perfectly revealing information. Here, the identification problem arises not from irrational trading but from a structural mismatch between the dimensionality of price and the dimensionality of the information it must convey. Investors who appear to be noise traders --- reacting to ``stale'' information, fixating on earnings, or trading on incomplete decompositions --- may instead be responding rationally to a problem that no amount of sophistication can resolve from price alone.
  ...
  The paper derives formal propositions characterizing the identification problem and its consequences for forecasting, showing that the constraint binds even under Bayesian optimality (Proposition \ref{prop:bayesian}). An agent-based simulation confirms these results computationally under bounded rationality: the market converges on the correct aggregate price while maintaining significant dispersion in individual parameter estimates.
...
* Model
  ...
  This section formalizes the identification problem and its consequences. The model requires only three assumptions: the structure of the security's value, the information available to investors, and the convergence of price.
...
\label{a:security}
The security has a true value $V = \sum_{i=1}^{n} \theta_i$, where $\boldsymbol{\theta} = (\theta_1, \ldots, \theta_n)$ is the vector of valuation parameters. Each $\theta_i$ is drawn independently from $\mathcal{N}(0, \sigma^2)$.
\end{assumption}

\begin{assumption}[Information Structure]
\label{a:information}
There are two classes of investor. \emph{Informed} investors know the true parameter vector $\boldsymbol{\theta}$. \emph{Uninformed} investors hold a prior $\hat{\boldsymbol{\theta}}^j = (\hat{\theta}_1^j, \ldots, \hat{\theta}_n^j)$ but do not observe any individual component directly.
\end{assumption}

\begin{assumption}[Price Convergence]
\label{a:convergence}
The market price $p$ converges to the true aggregate value: $p \to V = \sum_{i=1}^{n} \theta_i$. This is a maintained assumption consistent with models of competitive price formation in the presence of informed traders (citet:grossmanImpossibilityInformationallyEfficient1980, citet:godeAllocativeEfficiencyMarkets1993). The agent-based simulation in [[#a_sim][Appendix A]] confirms that this convergence obtains under specific trading and updating rules.
\end{assumption}

The model deliberately omits the specific mechanisms of trading and belief updating. Whether investors trade unit quantities or continuous demand schedules, whether they update by Bayesian inference or simple heuristics, does not affect the identification result. Aggregate price convergence is well established both theoretically (citet:grossmanImpossibilityInformationallyEfficient1980) and computationally (citet:godeAllocativeEfficiencyMarkets1993, and the simulation in [[#a_sim][Appendix A]]). The question is what can be learned from such a price once it has converged.

Note the absence of noise traders. In GS, noise traders are required to prevent price from perfectly revealing $\theta$, which would eliminate the incentive to become informed. In the multi-parameter setting, noise traders are unnecessary: the identification problem itself prevents perfect revelation of individual parameters. More generally, the identification problem does not depend on the specific price mechanism: any process that maps $n$ parameters to a single scalar price produces the same structural underidentification.
...
This result is immediate from the structure of the problem. The system has $n$ unknowns and one equation, leaving $n - 1$ degrees of freedom. This is not a matter of noise or bounded rationality; it is a constraint on the information content of the signal itself. The condition is trivially violated for any security whose value depends on more than one parameter --- which is to say, for any real security.

A natural question is whether a sufficiently sophisticated investor could overcome this constraint. The following proposition shows that even a fully Bayesian investor --- one who updates optimally given all available information --- cannot resolve the identification problem from price alone.

\begin{proposition}[Bayesian Non-Resolution]
\label{prop:bayesian}
Suppose an uninformed investor holds a prior $\boldsymbol{\theta} \sim \mathcal{N}(\boldsymbol{\mu}_0, \sigma_0^2 I_n)$ and observes the converged price $p = \sum_{i=1}^{n}\theta_i$. Her posterior is:
\begin{equation}
\boldsymbol{\theta} \mid p \;\sim\; \mathcal{N}\!\left(\boldsymbol{\mu}_0 + \frac{1}{n}\,\mathbf{1}(p - \mathbf{1}'\boldsymbol{\mu}_0),\;\; \sigma_0^2\left(I_n - \frac{1}{n}\,\mathbf{1}\mathbf{1}'\right)\right)
\end{equation}
where $\mathbf{1}$ is the $n$-vector of ones. The posterior has three properties:
\begin{enumerate}
\item The posterior mean for each component is $\hat{\theta}_i = \mu_{0,i} + \frac{1}{n}(p - \sum_j \mu_{0,j})$: the prior mean shifted uniformly by $\frac{1}{n}$ of the aggregate surprise. The investor has no basis for allocating the surprise differentially across components.
\item The posterior variance for each component is:
\begin{equation}
\text{Var}(\theta_i \mid p) = \sigma_0^2\,\frac{n-1}{n}
\end{equation}
which approaches $\sigma_0^2$ (the prior variance) as $n$ grows. A single price observation reduces individual-component uncertainty by only $\frac{1}{n}$.
\item The posterior is concentrated on the hyperplane $\mathcal{S}(p)$: the covariance matrix has rank $n - 1$, with the eigenvector $\mathbf{1}/\sqrt{n}$ (the ``aggregate direction'') having eigenvalue zero. The investor is certain of the sum but uncertain of the decomposition.
\end{enumerate}
\end{proposition}

\begin{proof}
By the standard Gaussian conjugate update for a linear observation $p = \mathbf{1}'\boldsymbol{\theta}$ with prior $\boldsymbol{\theta} \sim \mathcal{N}(\boldsymbol{\mu}_0, \sigma_0^2 I_n)$, the posterior mean is $\boldsymbol{\mu}_0 + \sigma_0^2\mathbf{1}(\mathbf{1}'\sigma_0^2 I_n\mathbf{1})^{-1}(p - \mathbf{1}'\boldsymbol{\mu}_0) = \boldsymbol{\mu}_0 + \frac{1}{n}\mathbf{1}(p - \mathbf{1}'\boldsymbol{\mu}_0)$ and the posterior covariance is $\sigma_0^2 I_n - \sigma_0^2\mathbf{1}(\mathbf{1}'\sigma_0^2 I_n \mathbf{1})^{-1}\mathbf{1}'\sigma_0^2 = \sigma_0^2(I_n - \frac{1}{n}\mathbf{1}\mathbf{1}')$. The diagonal entries of this matrix are $\sigma_0^2(1 - 1/n) = \sigma_0^2(n-1)/n$. The eigenvalues of $I_n - \frac{1}{n}\mathbf{1}\mathbf{1}'$ are $1 - n/n = 0$ (eigenvector $\mathbf{1}/\sqrt{n}$) and $1 - 0 = 1$ (multiplicity $n-1$, eigenvectors orthogonal to $\mathbf{1}$), confirming that the posterior is rank $n-1$ and concentrated on $\mathcal{S}(p)$.
\end{proof}

Proposition \ref{prop:bayesian} establishes that the non-identification result is not an artifact of any particular updating rule. The bottleneck is not how the investor processes the signal but how much information the signal contains. A single price carries exactly one dimension of information regardless of investor sophistication. Bayesian optimality distributes the aggregate information evenly across components, and the posterior point estimate remains dominated by the prior. The agent-based simulation in Section [[#results][4]] confirms that the same non-identification manifests under bounded rationality with a simple heuristic updating rule.

As discussed in Section [[#litreview][2]], the mathematical principle that a lower-dimensional signal space cannot encode a higher-dimensional state space was established by citet:radnerRationalExpectationsEquilibrium1979 and citet:jordanGenericExistenceRational1982, but their results concern multi-commodity economies rather than investor learning from a single security's price. Propositions \ref{prop:nonid} and \ref{prop:bayesian} apply this principle to the single-security setting and show that it binds even under optimal inference.
...
\begin{proof}
Since the $\theta_i$ are i.i.d.\ with variance $\sigma^2$ (Assumption \ref{a:security}):
\begin{equation}
...
\begin{proof}
(a) The investor has $k + 1$ linear constraints on $n$ unknowns: the aggregate constraint $\sum \theta_i = V_t$ and the $k$ component observations $\theta_j = c_j$ for $j \in K$. These are linearly independent since each component observation involves a different variable not constrained by any other observation (this holds generically; it would fail only if the same component were observed twice, which is ruled out by $K$ being a set). The solution set has dimension $n - (k + 1) = n - 1 - k$. When $k \geq n - 1$, the system has at least $n$ independent constraints, yielding a unique solution.
...
* Simulation Evidence
  ...
  The propositions above derive the identification problem and its consequences from the structure of the information environment alone, without specifying how investors trade or update beliefs. To verify that these results manifest in a concrete market setting --- and to visualize what non-identification looks like in practice --- this section reports results from an agent-based simulation.

** Simulation Design

  The simulation creates a market of $N = 500$ investors, of whom a fraction $\lambda = 0.6$ are informed (they know $\boldsymbol{\theta}$) and the remainder are uninformed (they hold independent random priors drawn from $\mathcal{N}(0,1)$). Each investor submits a unit buy or sell order based on whether her aggregate prior exceeds or falls below the current price. The price adjusts by $\pm 0.01$ per round based on net order imbalance. Uninformed investors update their priors by selecting a random parameter and partially revising it toward its value implied by the current price. The precise trading and updating rules are stated formally and implemented in full in [[#a_sim][Appendix A]].

  This updating rule is deliberately simple and suboptimal. As Proposition \ref{prop:bayesian} shows, the identification problem persists even under Bayesian optimality. The simulation demonstrates that it also persists under bounded rationality with a simple heuristic --- if anything, the non-identification is /more/ pronounced because the heuristic updating introduces additional dispersion beyond the structural minimum.
...
** Post-earnings-announcement drift and earnings fixation
  ...
  If this interpretation is correct, PEAD may not reflect cognitive underreaction or processing failure. Instead, it may reflect investors responding rationally to the fact that they cannot fully assess the information content of an earnings surprise relative to what was already in price. The drift occurs as supplementary signals arrive and help investors work out which components of value were already priced and which were not. These two accounts --- cognitive underreaction and structural non-identification --- make distinct cross-sectional predictions. Cognitive underreaction predicts that drift should diminish with investor sophistication and attention, whereas the identification mechanism predicts that drift should increase with the number of valuation components $n$ and the dispersion of their persistence rates, /regardless/ of investor sophistication. The multiplicative interaction $n \cdot \text{Var}_n(\boldsymbol{\alpha})$ predicted by Proposition \ref{prop:mse} is unique to the identification account and provides a direct empirical test.
...
** The value of supplementary signals: why components matter for predicting future prices
  ...
  These structural coefficients constrain the /shape/ of the solution set: they do not pin down a specific $\theta_i$, but they restrict how the components relate to the aggregate, narrowing the range of plausible decompositions. To see this concretely, suppose the solution set is the hyperplane $\theta_1 + \theta_2 = 10$. Without structural information, every point on this line is equally plausible. Now suppose an investor observes that the ERC for earnings ($\theta_1$) is approximately 0.9 --- that is, a unit change in $\theta_1$ moves price by 0.9 rather than 1.0. This implies that a change in $\theta_1$ partially offsets $\theta_2$ (perhaps because high earnings reduce a risk premium), constraining the /direction/ in which the true parameters lie along the hyperplane. The solution set remains one-dimensional, but the investor's posterior over it is no longer uniform --- the structural observation concentrates probability mass on decompositions consistent with the observed ERC.
...
* Conclusion
  This paper extends prior models, in which investors perfectly learn a single valuation parameter from price, to a setting in which price responds to multiple valuation parameters. The analysis shows that this creates an identification problem: a single price constrains investors to an \(n-1\)-dimensional hyperplane of parameter vectors, all of which are consistent with the observed price (Proposition \ref{prop:nonid}). This constraint binds even under Bayesian optimality: the posterior variance for each component is $\sigma_0^2(n-1)/n$, approaching the prior variance as $n$ grows (Proposition \ref{prop:bayesian}). When valuation components evolve at different rates, investors who cannot decompose the aggregate are forced to apply average persistence, generating forecast errors that increase in both the number of components and the dispersion of their persistence rates (Propositions \ref{prop:forecast}--\ref{prop:mse}). Supplementary signals reduce these errors, with each signal's value depending on which component it reveals (Proposition \ref{prop:collapse}, Corollary \ref{cor:marginal}). An agent-based simulation confirms these results under bounded rationality, showing that the market converges on the correct aggregate price while maintaining significant dispersion in individual parameter estimates. Aggregate market efficiency --- in which price reflects all available information --- is therefore distinct from the market efficiently pricing the individual informational components that compose that value.

  This identification constraint is qualitatively different from the impediments to learning in prior noisy rational expectations models (citet:grossmanImpossibilityInformationallyEfficient1980, citet:fischerReportingBias2000, citet:felthamPerformanceMeasureCongruity1994). In those settings, incomplete learning arises from exogenous noise perturbing a fundamentally identified signal; removing the noise restores perfect learning. Here, even without noise, a single price cannot reveal individual parameters when $n > 1$. The dimensional principle underlying this result is well established in general equilibrium theory (citet:radnerRationalExpectationsEquilibrium1979, citet:jordanGenericExistenceRational1982, citet:allenStrictRationalExpectations1982), but has not previously been applied to single-security investor learning or connected to the accounting literature's assumptions about how investors process earnings components. To be clear about what the paper does and does not claim: the analysis does not argue that markets are inefficient, nor that investors can never learn individual component values. Markets may well be efficient in the aggregate sense that price reflects all available information. What the analysis shows is that this aggregate efficiency is /logically insufficient/ to conclude that investors have formed efficient estimates of the individual components. Component learning requires supplementary signals beyond price, and the degree of component-level efficiency depends on the richness of the informational ecosystem --- disclosures, analyst coverage, and peer information channels --- not on aggregate price convergence alone.
...
** Simulation Implementation
  ...
  This appendix provides the complete implementation of the agent-based simulation reported in Section [[#results][4]], written in Clojure, a functional programming language whose immutable data structures and first-class functions make it well suited to modeling stateless agent interactions. The code serves two purposes: it provides a concrete, executable specification of the simulation, and it enables replication and extension of the results.

  The simulation implements three design choices beyond the model assumptions stated in Section [[#model][3]]:

  \emph{Market price mechanism.} The market maintains a scalar price $p_t$, initialized at $p_0 = V + \eta$ where $\eta \sim \mathcal{N}(0,1)$, and updated by $p_{t+1} = p_t + \delta \cdot \text{sgn}(\sum_j d_j^t)$ with $\delta = 0.01$.

  \emph{Trading rule.} Each investor submits a unit order $d_j^t \in \{-1, 0, 1\}$ based on whether her aggregate prior exceeds the current price.

  \emph{Belief updating.} Informed investors do not update. Uninformed investors update if price fails to move toward their prior: they select a parameter $k$ uniformly at random, compute the inferred value $\tilde{\theta}_k^j = p_t - \sum_{i \neq k}\hat{\theta}_i^j$, and average: $\hat{\theta}_k^{j,\text{new}} = (\hat{\theta}_k^j + \tilde{\theta}_k^j)/2$.

  This updating rule is deliberately simple. As Proposition \ref{prop:bayesian} demonstrates, the identification problem persists even under Bayesian optimality; the heuristic rule is adopted to show that it also manifests under bounded rationality, not because the results depend on it.

  Following these design choices, the implementation creates two types of investor. Operationally, a fully informed investor's prior is equal to the true value of the security. An uninformed investor's prior consists of independent draws from $\mathcal{N}(0,1)$. The following function creates either an informed or uninformed investor, depending on its arguments:
...
When called with an integer, /n/, the above function creates an uninformed investor with a randomly-generated prior containing /n/ parameters (Assumption \ref{a:information}). When called with a vector prior /v/ and with /fully?/ set to true, the function creates a fully informed investor whose prior is set to the vector prior. The function also contains an option to create a partially informed investor: if /fully?/ is set to false, the function randomly chooses to make the investor informed about one parameter in the vector prior. This option is included for potential extensions; all results reported in this paper use the fully informed/fully uninformed specification.
...
\singlespacing
#+BEGIN_SRC clojure :results value silent
  (def investor-num 500) ;; N = 500
  (def inform-frac 0.6) ;; lambda = 0.6
...
The following functions update the market price based on investor demand for shares. Investors submit unit orders $d_j^t \in \{-1, 0, 1\}$ based on whether their aggregate prior exceeds the current price, and the market adjusts price by $\pm \delta$ based on the sign of the net order imbalance.
...
#+BEGIN_SRC clojure :results value silent
  ...
  (defn- order-update
    [m]
    (let [;; d_j^t based on prior vs. price
          pe (reduce + (mapv (fn [a]
                               (let [prior (reduce + (prior-vals a))]
                                 (cond
                                   ...
                                   :else 0)))
                             (:investors m)))]
      ;; p_{t+1} = p_t + delta * sgn(sum of orders)
      (+ (:price m) (cond
                      (> pe 0) 0.01
                      (< pe 0) -0.01
                      :else 0)))
...
Informed investors do not update. Uninformed investors observe the period-over-period price change and decide to revise a parameter of their prior if the price fails to move towards what they deem to be the correct price. Otherwise, they assume that their prior is accurate.
...
*** Results Generation
  ...
  The following code generates the simulation results presented in Section [[#results][4]].
...
|-------------+----------------------------+----------------------------------------------------|
| Number of Parameters | Mean MSE | t-stat vs. $n = 2$ | t-stat vs. preceding $n$ |
|           1 |    0.000 |                                                |                                                  |
|           2 |    0.078 |                                                |                                                  |
|           3 |    0.109 |                                          4.426 |                                            4.426 |
|           4 |    0.129 |                                           7.34 |                                             2.85 |
